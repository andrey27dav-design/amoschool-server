// Диагностика нот для сделки Kommo #18287791
const KOMMO_TOKEN = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsImp0aSI6ImFiNmFlNzdiNjI4NWQ1ZmYwNmM0MmY4ZjQ2NjRhYmU0NGNkNDgzMWJlZTRhNDM2MzI0NjEyZGY1N2FjZGNkYzUwNjRlYTFjNDk2ZDQxN2E3In0..J-fe71D0ODNz_OvlZIJSP0dZ-DSN9qUUtCjDxmcm76l-X2oiKTrlQht6gPuPygEgbdezr-9DD5mqxdYAnVNi92eahqLTWAiVS9UmieiFc2duGnYwlKO3zJ44zd0hV_D29HTPawhNAjm3bur19FKYSV7teJKUCPdSVOkp_ew7I2TmEszKqvGTisWubplGz9wUROBefUzjCKGgMSXXhqZm27FHKE6RhAUTSagys6o09dxxh2gGktEXJ5BpoQ2Q3GX3y-I8YDPHs-R6bbxLz6SUol3s5fzobM9KNY1NgbNZ1297v43Wp2PUTLfgDY7fDX4d7zFjGFYlYjn-2XOvdFiTQA';
const KOMMO_BASE = 'https://helloshkolaonlinecom.kommo.com';

const DEAL_ID = 18287791;
const AMO_DEAL_ID = 22595523; // сделка из AMO которую показал пользователь

const AMO_TOKEN = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsImp0aSI6ImI5MGYyZjIxNTIxMGQxNGQ0MTg0NDRhODBkMzBjODlkOTcwMmU4NTBmNjk5YTg3ODk4ODQ5ZmNiMWM1ZWJlZmRjZjAwNGY5ZmQzNzA4Zjg1In0..HiQPlgnA5h_YEplP4MawVuktKA0wgKWT4Gag-JHIn3yt-0E-q7GO_At0L4ZSV044-R8r9qRFfl5IFUIzx1sB_xXGVdckukIYbjUpfUAy1iRChC2fGWJ7ATjZaR8sQT6tcBXB6wnDJCOoWZgEtJaOyUASvDm_TTltATuieUkSOJ5FQvc2ggfRZ4x_KjFCvAlmwoMeRJv0t3YUXv7PZ4DRJxR7CdW0SQge3hWFmDXi_HBW8e-eP2cLNp3hw_iA7r_xb1LGGYba3fS8_HzBJEnxxnoPiYnxywqK6Ug3ZxpO-SfXyI5IwrwM62q9W6ElZ5ZVPBjkpaKAOclnrO_SUfRlNA';
const AMO_BASE = 'https://houch.amocrm.ru';

async function fetchAll(url, token) {
  const res = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
  if (!res.ok) { console.log('HTTP', res.status, url); return null; }
  return res.json();
}

async function main() {
  console.log(`\n=== KOMMO ноты для сделки #${DEAL_ID} ===`);
  const kData = await fetchAll(`${KOMMO_BASE}/api/v4/leads/${DEAL_ID}/notes?limit=50`, KOMMO_TOKEN);
  if (kData && kData._embedded && kData._embedded.notes) {
    const notes = kData._embedded.notes;
    console.log(`Всего нот в Kommo: ${notes.length}`);
    notes.forEach(n => {
      const d = new Date(n.created_at * 1000).toISOString().substring(0,16);
      console.log(`  id=${n.id} type=${n.note_type} created_by=${n.created_by} created_at=${d}`);
      if (n.params && n.params.text) console.log(`    text: ${n.params.text.substring(0,80)}`);
    });
  } else {
    console.log('Нет нот или ошибка');
  }

  console.log(`\n=== AMO ноты для сделки #${AMO_DEAL_ID} ===`);
  const aData = await fetchAll(`${AMO_BASE}/api/v4/leads/${AMO_DEAL_ID}/notes?limit=50`, AMO_TOKEN);
  if (aData && aData._embedded && aData._embedded.notes) {
    const notes = aData._embedded.notes;
    console.log(`Всего нот в AMO: ${notes.length}`);
    notes.forEach(n => {
      const d = new Date(n.created_at * 1000).toISOString().substring(0,16);
      console.log(`  id=${n.id} type=${n.note_type} created_by=${n.created_by} created_at=${d}`);
      if (n.params && n.params.text) console.log(`    text: ${n.params.text.substring(0,80)}`);
    });
  } else {
    console.log('Нет нот или ошибка');
  }

  // Проверим состояние индекса - есть ли сделка 18287791 в notes_leads
  const fs = require('fs');
  try {
    const idx = JSON.parse(fs.readFileSync('/var/www/amoschool/backend/backups/migration_index.json', 'utf8'));
    const nLeads = idx.notes_leads || {};
    const count = Object.keys(nLeads).length;
    console.log(`\n=== ИНДЕКС ===`);
    console.log(`notes_leads записей: ${count}`);
    if (count > 0) {
      console.log('Первые 5:', Object.entries(nLeads).slice(0,5).map(([k,v])=>`AMO ${k}→Kommo ${v}`).join(', '));
    }
  } catch(e) {
    console.log('Не могу читать индекс:', e.message);
  }
}

main().catch(console.error);
