    ...
    updateState({ step: 'Перенос комментариев сделок...' });
    {
      ...
      try {
        ...
        for (const [kId, notes] of Object.entries(_batchLeadNotesGrouped)) {
          ...
          const n = notes.map(note => { const s = sanitizeNoteParams(note); return { entity_id: kId, note_type: s.note_type, params: s.params, created_by: 12739795 }; });
          try {
      ...
      for (const c of (aLead._embedded?.contacts || [])) {
        ...
        try {
          ...
          if (_bCNotesToCreate.length > 0) {
            ...
            const n = _bCNotesToCreate.map(note => { const s = sanitizeNoteParams(note); return { entity_id: kContactId, note_type: s.note_type, params: s.params, created_by: 12739795 }; });
