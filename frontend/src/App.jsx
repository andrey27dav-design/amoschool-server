...
const MIGRATION_PLAN = [
  { step: 1, title: '⚠️ Синхронизация этапов воронки (разово)', desc: 'Вкладка «Воронки» → выберите воронку amo CRM (Школа/Репетиторство) и воронку Kommo → «Синхронизировать этапы». Делается один раз. Без этого сделки попадут в неправильные этапы.' },
  ...
  { step: 5, title: 'Пакетный перенос', desc: 'Выберите размер пакета (10 / 50 / 100 / 200 / ВСЕ) → «▶️ Перенести». Порядок внутри пакета: компании → контакты → сделки → задачи → комментарии. Зелёные числа (✅) растут по мере переноса. Следующий нажим «Перенести» продолжит с того места, где остановились.' },
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="plan-intro">
                ...
                в <strong>Kommo CRM</strong> (воронка «Deferred / Russianlanguadge Department»).
                Исходные данные сохраняются — ничего не удаляется автоматически.
              </div>
              <div className="plan-warning">
                ⚠️ Первый запуск: сначала Воронки → Менеджеры → Поля — только потом Дашборд.
                Без сопоставления менеджеров сделки окажутся без ответственного!
              </div>
              ...
              <div className="plan-section">
                <h3>📊 Что означают счётчики</h3>
                <ul>
                  <li><strong>Серые числа 📥</strong> — загружено из amo CRM (всего в кэше)</li>
                  <li><strong>Зелёные числа ✅</strong> — перенесено в Kommo (с момента последнего сброса)</li>
                  <li>Расхождение серого и зелёного — норма, если перенос ещё не завершён</li>
                </ul>
              </div>
              ...
              <div className="plan-section">
                ...
                <ul>
                  <li><strong>«▶️ Перенести»</strong> — продолжить с того места где остановились</li>
                  <li><strong>«🔁 Сбросить счётчик»</strong> — начать очередь заново (зелёные числа обнулятся)</li>
                  <li><strong>«🔍 Только необработанные»</strong> — убрать уже перенесённые из кэша + сбросить офсет</li>
                  <li><strong>«⏸ Пауза»</strong> — остановить после текущей сделки</li>
                </ul>
              </div>
              ...
              <div className="plan-section">
                ...
                <ul>
                  <li><strong>Завис / ошибка API</strong> — нажмите «Перенести» снова, продолжит с той же точки. Дублей не будет.</li>
                  <li><strong>Баннер «CRASH DETECTED»</strong> — нажмите «▶ Продолжить» для возобновления</li>
                  <li><strong>Перенесены лишние сделки</strong> — «↩ Откатить пакет» удалит последний пакет из Kommo</li>
                  <li><strong>Полный откат</strong> — кнопка «Откатить всё» в разделе отката</li>
              ...
              <div className="plan-section">
                ...
                <ul>
                  ...
                  <li><strong>🔀 Воронки</strong> — синхронизация этапов (разово)</li>
                  <li><strong>👥 Менеджеры</strong> — сопоставление менеджеров (разово, обязательно!)</li>
                  <li><strong>🔧 Поля</strong> — создание кастомных полей в Kommo (разово)</li>
                  <li><strong>💾 Бэкапы</strong> — резервные копии данных</li>
                </ul>
              </div>
              ...
              <div className="plan-section">
                ...
                <ol>
                  <li>Зелёные счётчики на дашборде соответствуют ожиданиям</li>
                  <li>В воронке Kommo «Deferred» — сделки с правильными этапами и менеджерами</li>
                  <li>Открыть несколько карточек — проверить поля, задачи, комментарии в таймлайне</li>
                  <li>Данные в amo CRM удалить вручную только после полной проверки</li>
                </ol>
              </div>

              <div className="plan-section" style={{background:'#1e293b',borderRadius:'8px',padding:'12px',fontFamily:'monospace',fontSize:'12px'}}>
                <h3 style={{marginTop:0}}>⚡ Шпаргалка: быстрый старт</h3>
                <div style={{color:'#94a3b8',lineHeight:'1.8'}}>
                  <div><span style={{color:'#f59e0b'}}>Первый раз:</span> Воронки → Менеджеры ✅ → Поля ✅</div>
                  <div><span style={{color:'#10b981'}}>Каждая сессия:</span> Загрузить данные → выбрать размер → Перенести</div>
                  <div><span style={{color:'#60a5fa'}}>Продолжить прошлую:</span> просто нажать «Перенести» (офсет сохранён)</div>
                  <div><span style={{color:'#a78bfa'}}>Только новые:</span> «🔍 Только необработанные» → «🔁 Сбросить» → «Перенести»</div>
                </div>
              </div>
            </div>
          </div>
